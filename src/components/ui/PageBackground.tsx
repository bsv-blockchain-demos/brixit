...
export function PageBackground({ children, className, ...props }: PageBackgroundProps) {
  return (
    <div className={cn('relative isolate bg-background', className)} {...props}>
      ...
      {/* Decorative wallpaper — light mode only. Dark mode stays a clean solid
          surface (no backdrop). */}
      <div
        aria-hidden="true"
        className="fixed inset-0 -z-10 bg-cover bg-center pointer-events-none select-none opacity-[0.55] dark:hidden"
