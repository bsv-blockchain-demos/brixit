 ...
 *
 * Desktop (≥641px) is unchanged. At ≤640px the filters collapse to a Search +
 * Filters button, and the full panel opens on demand in a bottom sheet — same
...
export default function DataBrowserFilters({ fromLeaderboard = false }: { fromLeaderboard?: boolean }) {
  ...
  const activeCount = getActiveFilterList(filters, isAdmin).length;
  ...
  // and the mobile bottom sheet (the grid collapses to one column on narrow).
  const renderFilterFields = () => (
    <>
      ...
      {/* Row 3 — toggles. In the mobile sheet, each is a full-width row with the
          switch right-aligned; desktop keeps the inline layout. */}
      <div className={isNarrow ? "space-y-4 pt-1" : "flex flex-wrap items-center gap-6 pt-1"}>
  ...
  // ── Mobile (≤640px): collapsed Search + Filters button, panel in a sheet ──
  if (isNarrow) {
    return (
      <>
        <div className="p-3 border-b border-hairline flex gap-2 items-center">
          ...
          >
            ...
            <span>Filters</span>
            {activeCount > 0 && (
              <span className="inline-flex items-center justify-center min-w-[20px] h-5 px-1.5 rounded-full bg-blue-mid text-white text-xs font-semibold">
                {activeCount}
              </span>
            )}
          </Button>
        ...
        <Drawer open={sheetOpen} onOpenChange={setSheetOpen}>
          <DrawerContent className="max-h-[88vh] bg-card text-card-foreground">
            <div className="relative flex items-center justify-center px-4 pt-2 pb-3 shrink-0">
              <span className="text-base font-semibold text-text-dark">Filters</span>
              {activeCount > 0 && (
                <button
                  onClick={clearFilters}
                  className="absolute right-4 text-sm font-medium text-action-primary"
                >
                  Clear ({activeCount})
                </button>
              )}
      ...
      <div className="p-4 border-b border-hairline flex flex-col md:flex-row gap-4 items-center">
        ...
        {filterSummary !== 'No active filters' && (
          <Button
            variant="ghost"
            onClick={clearFilters}
            className={`text-destructive ${fromLeaderboard ? 'lb-desktop-only' : ''}`}
          >
